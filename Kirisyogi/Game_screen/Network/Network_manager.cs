...
{
    public static bool is_end_game = false;
    private static int i;
    private static RoomOptions roomOptions_data;

    public Photon.Realtime.RoomOptions Return_roomOption() {
        // ルームのカスタムプロパティの初期値
    ...
    private void Start() {
        ...
        PhotonNetwork.NickName = $"{Game_data.user_name}/{DateTime.Now.ToString("HH:mm:ss")}";
        i = 1;
    ...
    public override void OnConnectedToMaster() {
        // from OnLeftRoom
        print("マスターサーバー接続完了");
        if (is_end_game) {is_end_game = false; print("サーバー待機");}
        else {
    ...
    public override void OnJoinedRoom() {
        ...
        Main.Create_init();
        print("通信待機中");
        //f.print_r(PhotonNetwork.PlayerList);
