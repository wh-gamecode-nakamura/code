...
public class Game_obj: MonoBehaviour
{
    ...
    public Chara_sc now_chara_sc = null;
    ...
    public bool is_start_turn = true;
        ...
        if (!now_chara_sc) {
            ...
            if (!hover_chara_sc) {
                ...
                field.disp_movable_pos(chara.field_chara_li, hover_chara_sc, 
                    Field_color.can_move, hand.hand_li[p_num].Count);
    ...
    private void Choice_mark_proc() {
        ...
        int[] move_chara_li = new int[] { //** {1, 3, 4, 1} 座標は移動後の相手目線での座標
            now_chara_sc.chara_id, (Coord.x+1)-dest[0], (Coord.y+1)-dest[1], now_chara_sc.dup_id};

        //$b 手札から移動する場合
        if (!field.Is_in_field(f.return_pos(now_chara_sc.obj))) {
            now_chara_sc.Dup_check(chara.field_chara_li[now_chara_sc.p_num], now_chara_sc, false);
            hand.hand_li[now_chara_sc.p_num].Remove(now_chara_sc);
        }
        ...
        // 移動１
        now_chara_sc.pos = new int[] {dest[0], dest[1]};
        field.Move_chara(now_chara_sc);
    ...
    private void Choice_chara_proc() {
        now_chara_sc = click_obj.GetComponent<Chara_sc>();
        if (now_chara_sc.mine) {
            // 移動先表示
            field.disp_movable_pos(chara.field_chara_li, now_chara_sc, 
    ...
    public void Move_piece(int[] dest) {
        //$b 移動先に駒があった場合
        if (field.field_chara_data[dest[1], dest[0]]) {
            Chara_sc delete_chara_sc = field.field_chara_data[dest[1], dest[0]];
            ...
            Chara_sc chara_sc = f_k.Instance_chara(
                p_num, now_chara_sc.p_num, hand_chara_id, new int[] {0, 0});
        ...
        if (((dest[1] > Coord.y - Coord.mistRow) && (now_chara_sc.mine == true)) ||
        ((dest[1] < Coord.o + Coord.mistRow) && (now_chara_sc.mine == false))) {
            //$p 霧の処理
            field.field_data[dest[1], dest[0]].mist_st = 0;

            //$b その駒が成っていない場合
            if (!now_chara_sc.promote_state) {
                //$b 成る駒の場合
                if (now_chara_sc.promote_id != 0) {
                    //$p 駒が成る処理
                    now_chara_sc = Consists_piece();;
                }
            }
        }
        //$b プレイヤー毎の相手の端に移動した場合
        if (((dest[1] == Coord.y) && (now_chara_sc.mine == true)) ||
        ((dest[1] == Coord.o) && (now_chara_sc.mine == false))) {
            //$b 王が移動した場合
            if (now_chara_sc.is_king) {
                win_p_num = now_chara_sc.p_num;
    ...
    private Chara_sc Consists_piece() {
        ...
        Chara_sc consists_chara_sc = f_k.Instance_chara(
            p_num, now_chara_sc.p_num, now_chara_sc.promote_id, now_chara_sc.pos);
        consists_chara_sc.obj.transform.position = now_chara_sc.obj.transform.position;
        consists_chara_sc.origin_id = now_chara_sc.chara_id;
        consists_chara_sc.cost = now_chara_sc.cost;
        consists_chara_sc.promote_state = true;
        
        // 
        chara.field_chara_li[now_chara_sc.p_num].Remove(now_chara_sc);
        chara.field_chara_li[now_chara_sc.p_num].Add(consists_chara_sc);
        Destroy(now_chara_sc.obj);
    ...
    public void Ref_recv(int[] move_chara_li) {
        ...
        now_chara_sc = chara.field_chara_li[(p_num+1)%2].Find(
            chara_sc => chara_sc.chara_id == move_chara_li[0] & 
            chara_sc.dup_id == move_chara_li[3]);
        // chara_idから手札検索
        if (!now_chara_sc) {
            now_chara_sc = hand.hand_li[(p_num+1)%2].Find(
                chara_sc => chara_sc.chara_id == move_chara_li[0]);
        }

        //$p 手札から移動する場合
        if (!field.Is_in_field(now_chara_sc.pos)) {
            now_chara_sc.Dup_check(chara.field_chara_li[now_chara_sc.p_num], now_chara_sc, false);
            hand.hand_li[now_chara_sc.p_num].Remove(now_chara_sc);
        }
        // 移動１
        now_chara_sc.pos = new int[] {move_chara_li[1], move_chara_li[2]};
        Move_piece(now_chara_sc.pos);
        field.Move_chara(now_chara_sc);
